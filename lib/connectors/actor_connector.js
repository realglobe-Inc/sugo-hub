...
class ActorConnector extends Connector {
  ...
  handleConnection (socket, scope) {
    ...
    async function broadcastToCallers (callerKeys, event, data) {
    ...
    socket.on(HI, s.ack(async function handleHi (data = {}) {
      let {key, force} = data
    ...
    socket.on(BYE, s.ack(async function handleBye (data = {}) {
      let {key} = data
    ...
    socket.on(SPEC, s.ack(async function handleSpec (data = {}) {
      let {name, spec} = data
      if (!name) {
    ...
    socket.on(DESPEC, s.ack(async function handleDespec (data = {}) {
      let {name} = data
      if (!name) {
