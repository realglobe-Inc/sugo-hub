 ...
 * @parma {number} [options.invalidateInterval=20000] - Interval for invalidate loop
...
function sugoCloud (options = {}) {
  ...
  port = port || 3000
  invalidateInterval = invalidateInterval || 20000
  let server = sgServer({
    endpoints,
    middlewares,
    context,
  ...
  return co(function * () {
    ...
    let _invalidateTimer = setInterval(() => invalidate(), invalidateInterval).unref()
