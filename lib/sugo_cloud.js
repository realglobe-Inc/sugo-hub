...
function sugoCloud (options = {}) {
  ...
  let spotService = new SpotService(storage, { prefix: 'sg:spot' })
  let terminalService = new TerminalService(storage, { prefix: 'sg:terminal' })
  ...
  return co(function * () {
    ...
    wsServer.of(SPOT_URL).on(CONNECTION, (socket) => {
      ...
      socket.on(PIPE, (data) => {
        co(function * () {
          ...
          let spot = yield spotService.getSpot(key, { strict: true })
    ...
    wsServer.of(TERMINAL_URL).on(CONNECTION, (socket) => {
      ...
      socket.on(JOIN, ack(function * joinSpot (data) {
        let spot = yield spotService.getSpot(data.key, { strict: true })
        ...
        yield spotService.saveSpot(spot.key, spot)
      ...
      // Cleanup sockets
      socket.on(DISCONNECT, () => co(function * cleanup () {
        let terminal = terminalService.getTerminal(id)
        for (let key of terminal.spots) {
          let spot = yield spotService.getSpot(key)
          if (spot) {
            delete spot.terminals[ id ]
            yield spotService.saveSpot(spot.key, spot)
          }
        }
        yield terminalService.delTerminal(id)
