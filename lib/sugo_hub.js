...
function sugoHub (options = {}) {
  let { port, prefix, invalidateInterval, endpoints, middlewares, context, keys } = defaults(options, {
    port: 3000,
    prefix: 'sugo-hub',
  ...
  if (storageConfig.redis) {
    ...
    wsServer.adapter(socketIORedis({ host, port, key: `${prefix}:socket.io` }))
  }
  ...
  return co(function * () {
    ...
    function invalidateActors () {
      return co(function * () {
        ...
        for (let key of Object.keys(invalidated)) {
          ...
          if (socket) {
            debug(`Invalidate invalid actor: ${key} (socket: ${socket.id})`)
    ...
    function invalidateCallers () {
      return co(function * () {
        let invalidated = yield callerService.invalidate((caller) => {
        ...
        for (let key of Object.keys(invalidated)) {
          ...
          if (socket) {
            debug(`Invalidate invalid caller: ${key} (socket: ${socket.id})`)
