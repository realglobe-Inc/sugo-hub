...
class ActorConnector extends Connector {
  ...
  handleConnection (socket, scope) {
    ...
    socket.on(SPEC, this.ack(async (data = {}) => {
      ...
      await broadcastToCallers(actor.callers, SPEC, data)
    ...
    socket.on(DESPEC, this.ack(async (data = {}) => {
      ...
      await broadcastToCallers(actor.callers, DESPEC, data)
    ...
    socket.on(RESULT, async (data) => {
      ...
      process.nextTick(() =>
        invocation && callerIO.to(invocation.callerSocket).emit(RESULT, data)
      )
    })
    ...
    socket.on(DISCONNECT, async () => {
      ...
      const actor = await actorService.findBySocketId(id)
      if (!actor) {
        return
      }
      if (actor.callers) {
        for (const callerKey of actor.callers) {
          ...
          if (caller) {
            ...
            for (const invocation of invocations) {
              ...
              const result = actorGoneInvocationError(invocation, actor)
              process.nextTick(() =>
        ...
        await broadcastToCallers(actor.callers, NOTICE, {
          name: ACTOR_GONE,
          data: {
            key: actor.key,
            at: new Date()
